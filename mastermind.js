...
if (laenge!=null){       // #### Mögl. Abbruch bei imput von Länge ####
    for (let z=0; z<4; z++) {               // Definieren der zu erratenden Zahlen
        ergebnisBackup[z] = prompt("Ergebnis 𝗦𝘁𝗲𝗹𝗹𝗲 "+(z+1)+":",0)
        if (ergebnisBackup[z]===null){
            break
        }
    }

    if (! ergebnisBackup.includes(null)){       // #### Mögl. Abbruch bei imput von ErgebnisBackup ####
        for (let i=0; i<laenge; i++) {
            if (tip.includes(null)){break}      //workaround: Runden können sonst nicht abgebrochen werden
            ...
            if (! tip.includes(null)){      // #### Mögl. Abbruch bei imput von tip ####
                if (crit<4 && i<laenge){
